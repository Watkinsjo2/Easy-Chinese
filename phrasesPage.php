...
<title>EZ Chinese Phrases List</title>
...
$sql = "SELECT phrase, translation FROM chinesephrase";
...
while( $row = mysqli_fetch_array( $result)){
	$transList[] = $row['translation']; // Inside while loop
	$phraseList[] = $row['phrase'];
}

echo "<div style=\"margin-left:200px; margin-bottom:10px;\">";
echo "<div style=\"float:left;\">";
$half = ceil(mysqli_num_rows($result)/2);
for($i = 0; $i < $half; $i++){
	echo  "<ul>\n";
	echo  "<li class=\"userText\">" .$transList[$i] ." / " .$phraseList[$i] . "</li>\n"; 
	echo  "</ul>";
}
echo "</div>";

echo "<div style=\"float:right; margin-right:25px;\">";
for($i = 0; $i < $half; $i++){
	$val = $i + $half;
	echo  "<ul>\n";
	echo  "<li class=\"userText\">" .$transList[$i + $half] ." / " .$phraseList[$i + $half] . "</li>\n"; 
	echo  "</ul>";
}
echo "</div>";
echo "</div>";
